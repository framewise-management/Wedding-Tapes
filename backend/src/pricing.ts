...
export function calculatePricing(input: PricingInput): PricingResult {
  const packageTotal = input.packages.reduce((sum, p) => sum + p.quantity * p.unitPrice, 0);
  const serviceTotal = input.items
    .filter((i) => !i.isOptional)
    .reduce((sum, i) => sum + i.quantity * i.unitPrice, 0);
  const subtotal = packageTotal + serviceTotal;

  const rawDiscount = calculateDiscount(subtotal, input.discountType, input.discountValue);
