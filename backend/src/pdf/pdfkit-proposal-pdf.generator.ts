...
@Injectable()
export class PdfKitProposalPdfGenerator implements PdfGenerator {
  async generateProposalPdf(proposal: Proposal, business: Business): Promise<Buffer> {
    ...
    this.renderHeader(doc, proposal, business);
    this.renderCustomer(doc, proposal);
    this.renderWedding(doc, proposal);
    this.renderLineItems(doc, 'Selected Packages', proposal.packages.map((p) => ({
    ...
    this.renderLineItems(doc, 'Services', includedItems.map((i) => ({
      name: i.quantity > 1 ? `${i.serviceName} x ${i.quantity}` : i.serviceName,
      description: i.description,
      amount: i.total,
    })));
    this.renderLineItems(doc, 'Optional Services (not included in total)', optionalItems.map((i) => ({
      name: i.quantity > 1 ? `${i.serviceName} x ${i.quantity}` : i.serviceName,
      description: i.description,
      amount: i.total,
    })));
    this.renderPricing(doc, proposal);
    this.renderTerms(doc, business);
    this.renderFooter(doc, business);
  ...
  private renderHeader(doc: PDFKit.PDFDocument, proposal: Proposal, business: Business): void {
    const startY = doc.y;
    ...
    doc
      ...
      .font('Helvetica')
      .fillColor('#6b6375')
      .text(`Date: ${formatDate(proposal.createdAt)}`, AMOUNT_X - 100, doc.y, { width: 235, align: 'right' });
    if (proposal.validUntil) {
      doc.text(`Valid until: ${formatDate(proposal.validUntil)}`, AMOUNT_X - 100, doc.y, { width: 235, align: 'right' });
    }
    doc.fillColor('#08060d');
  ...
  private renderCustomer(doc: PDFKit.PDFDocument, proposal: Proposal): void {
    this.sectionHeading(doc, 'Customer');
    doc.fontSize(11).text(proposal.customer.name, PAGE_MARGIN, doc.y, { width: CONTENT_WIDTH });
    doc.fontSize(10).fillColor('#6b6375').text(proposal.customer.phone, PAGE_MARGIN, doc.y, { width: CONTENT_WIDTH });
    if (proposal.customer.email) doc.text(proposal.customer.email, PAGE_MARGIN, doc.y, { width: CONTENT_WIDTH });
    doc.fillColor('#08060d');
  }

  private renderWedding(doc: PDFKit.PDFDocument, proposal: Proposal): void {
    ...
    if (proposal.numberOfDays != null) {
      doc
        .fontSize(10)
        .fillColor('#6b6375')
        .text(`${proposal.numberOfDays} day${proposal.numberOfDays === 1 ? '' : 's'}`, PAGE_MARGIN, doc.y, { width: CONTENT_WIDTH });
      doc.fillColor('#08060d');
    }
  }
  ...
  private renderLineItems(
    ...
    for (const item of items) {
      ...
      doc
        ...
        .font('Helvetica')
        .fillColor('#08060d')
        .text(money(item.amount), AMOUNT_X, startY, { width: AMOUNT_WIDTH, align: 'right', lineBreak: false });

      doc.y = startY;
      doc.fontSize(11).font('Helvetica-Bold').text(item.name, PAGE_MARGIN, startY, { width: NAME_WIDTH });
      if (item.description) {
        doc.fontSize(9.5).font('Helvetica').fillColor('#8b8590').text(item.description, PAGE_MARGIN, doc.y, { width: NAME_WIDTH });
        doc.fillColor('#08060d');
      }
      doc.moveDown(0.5);
    }
  ...
  private renderPricing(doc: PDFKit.PDFDocument, proposal: Proposal): void {
    this.sectionHeading(doc, 'Pricing Summary');
    ...
    if (proposal.discountAmount > 0) {
      const label = proposal.discountType === DiscountType.PERCENTAGE
  ...
  private renderTerms(doc: PDFKit.PDFDocument, business: Business): void {
    if (!business.defaultTerms) return;
    this.sectionHeading(doc, 'Terms & Conditions');
    doc.fontSize(9.5).fillColor('#6b6375').text(business.defaultTerms, PAGE_MARGIN, doc.y, { width: CONTENT_WIDTH });
    doc.fillColor('#08060d');
  }

  private renderFooter(doc: PDFKit.PDFDocument, business: Business): void {
    doc.moveDown(1.5);
    doc
      .fontSize(8.5)
      .fillColor('#8b8590')
      .text(`Generated by ${business.name} on ${formatDate(new Date())}`, PAGE_MARGIN, doc.y, { width: CONTENT_WIDTH, align: 'center' });
  }
}
