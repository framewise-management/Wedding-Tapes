...
- `backend/` — NestJS + TypeScript (native ESM — `"type": "module"`, `nodenext` resolution, relative imports need `.js` extensions even in `.ts` files). Package manager **pnpm**. API routes are prefixed `/api` (`main.ts` `setGlobalPrefix`), except `/` and `/health` which stay unprefixed ops endpoints.
  - Phase 0: `@nestjs/config`, `@nestjs/typeorm` + TypeORM 1.x + `pg`, global `ValidationPipe`, a global `HttpExceptionFilter` shaping errors as `{ error: { code, message } }` (SRS §12), CORS. DB is **Supabase Postgres** via a single `DATABASE_URL` env var, pointed at Supabase's transaction pooler (port 6543, host `*.pooler.supabase.com`) rather than the direct connection — the pooler is what handles many short-lived serverless connections without exhausting Supabase's connection limit. Supabase's pooler presents its own CA chain (`Supabase Root 2021 CA`) that isn't in Node's bundled trust store, so cert verification needs the CA passed explicitly via a second env var, `DATABASE_CA_CERT` (the PEM content, downloaded from the Supabase dashboard's Database Settings → SSL Configuration), wired into both `TypeOrmModule.forRootAsync` (`app.module.ts`) and `AppDataSource` (`data-source.ts`) as `ssl: { ca: ... }`. **Do not put `sslmode=require` (or any `sslmode`) in `DATABASE_URL`** — `pg`'s connection-string parser treats those as an alias for `verify-full` and silently builds its own `ssl` config from Node's default trust store, overriding the explicit `ssl.ca` object entirely and reintroducing `SELF_SIGNED_CERT_IN_CHAIN`. Never fall back to `ssl: { rejectUnauthorized: false }` to dodge this — it disables cert verification instead of fixing it.
...
pnpm run migration:run  # backend: apply pending TypeORM migrations (Supabase)
